...
class OrderModel
{
  ...
  public function saveOrder($orders, $userId, $totalAmount)
  {
    ...
    $sql = "INSERT INTO orders (User_Id, CreationTimestamp) 
    VALUES ( ?, NOW() )";
    $values = [$userId];
    $orderId = $database->executeSql($sql, $values);
    foreach ($orders as $order) {
      $sql = "INSERT INTO orderline (Quantity_ordered	,Product_Id, Order_Id, PriceEach) 
      VALUES (?, ?, ?, ?)";
      $values = [$order->quantity, $order->id, $orderId, $order->safePrice];
      $database->executeSql($sql, $values);
    }
    $sql = "UPDATE orders 
    SET TotalAmount=?, TaxAmount=?, TaxRate=? 
    WHERE Id= ?";
    $taxRate = 20;
